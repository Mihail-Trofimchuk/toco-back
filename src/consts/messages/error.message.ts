export const ACCOUNT_HAS_BEEN_DISABLED: string = "Account has been disabled";
export const CURRENT_PASSWORD_IS_INCORRECT: string =
  "Current password is incorrect";
export const EMAIL_OR_PASSWORD_IS_INCORRECT: string =
  "Email or password is incorrect";
export const EMAIL_DOES_NOT_EXIST: string = "Email doesn't exist";
export const EMAIL_ALREADY_EXISTS: string = "Email already exist";
export const LOGIN_VALUES_ARE_INVALID: string = "Login values are invalid";
export const TOKEN_IS_INVALID: string = "Token is invalid";
export const TOKEN_IS_BLACKLISTED: string = "Token is blacklisted";
export const TOKEN_DOES_NOT_EXIST: string = "Token does not exist";
export const TODO_DOES_NOT_EXIST: string = "Todo does not exist";
export const USER_DOES_NOT_EXIST: string = "User doesn't exist";
export const UNAUTHORIZED_ACTION: string = "You are not authorized to perform this action";
export const RESOURCE_NOT_FOUND: string = "Resource not found";